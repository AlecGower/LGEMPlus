...
with open(results_root / 'single_gene_deletions.txt', 'w') as fo:
    ...
    n_genes = int(os.popen('wc {}'.format(theory_root / 'gene_list.txt')).read().split()[0])
    with open(theory_root / "gene_list.txt") as fi:
        for ln in tqdm(fi, total=n_genes):
            ...
            if orf not in base_deletants and name not in base_deletants:
                # Calculate deletion
                deletion_stream = os.popen(
                    'helpers/gene_knockout_simple.sh {} {} {}|grep -c "% SZS status Satisfiable"'.format(
                        orf,
                        theory_root / "genes.p",
                        " ".join(
                            [
                ...
                print(orf, name, deletion_stream.read().strip(), sep='\t', file=fo)
