...
while read -r hyp add_clause_component; do
	...
	if [[ $hypCount > $hyp ]]; then
		...
		# $IPROVER_HOME"/iproveropt" --proof_out false --sat_out_model none --sat_out_clauses false --sat_out_model none ${@:2} ""$output_directory"/"$1"_hypothesis_"$hypCount".p" |grep "% SZS status"
		$IPROVER_HOME"/iproveropt" --schedule none --preprocessing_flag false --instantiation_flag false --superposition_flag false --resolution_flag true --res_prop_simpl_given false --res_to_prop_solver none --prop_solver_per_cl 0 --proof_reduce_dot "[all_neg;unit]" --proof_dot_file ""$output_directory"/"$1"_hypothesis_"$hypCount".dot" ${@:2} ""$output_directory"/"$1"_hypothesis_"$hypCount".p" >/dev/null
...
done < <(helpers/abduction/get_hypotheses_only_from_sat.pl $out_fp | sort -n)
if [[ $hypCount == $hyp ]]; then
